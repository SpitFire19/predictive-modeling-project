...
sum(complete.cases(Data0[, all.vars(as.formula(equation))]))
...
equation <- Net_demand~  Time + toy + Temp + Load.1 + Load.7 + Temp_s99 + as.factor(WeekDays) + BH + Temp_s95_max + Temp_s99_max + Summer_break  + Christmas_break + 
  Temp_s95_min +Temp_s99_min + DLS 
...
rmse(y=Data0[sel_b,]$Net_demand, ychap=gbm0.forecast)
...
quant <- qnorm(0.94, mean= mean(res), sd= sd(res))
quant <- quantile(res,0.95)
quant
pred <- gbm1.forecast+quant 
pinball_loss(y=Data0$Load[sel_b], pred, quant=0.8, output.vect=FALSE)

gbm1.forecast_test <- predict(gbm1,n.trees=NTreeOpt,single.tree=FALSE,newdata=Data1)
y_test <- c(Data1$Load, NA)
y_test[395] <- gbm1.forecast_test[395]
y_test = y_test[!is.na(y_test)]
y_test
pinball_loss(y=y_test, gbm1.forecast_test + quant, quant=0.8, output.vect=FALSE)
...
pinball_loss(y=Data0$Load[sel_b], xgb1.forecast, quant=0.8, output.vect=FALSE)
...
pinball_loss(y=Data0$Load[sel_b], xgb2.forecast, quant=0.8, output.vect=FALSE)
